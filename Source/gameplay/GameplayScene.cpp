...
// Sound macros
#define PLAY(name) if (ax::UserDefault::getInstance()->getBoolForKey("audioEnabled", true)) {   \
AudioEngine::play2d(name, false);                                                               \
}
...
bool GameplayScene::init() {
    ...
    if (const auto gameboard = NodeUtils::getNodeByName(mRoot, "gameboard")) {
        const auto visibleSize = ax::Size(gameboard->getContentSize().width, gameboard->getContentSize().height);
    ...
    // Create score
    if (auto gameScore = dynamic_cast<ax::ui::Text*>(NodeUtils::getNodeByName(mRoot, "score"))) {
    ...
    if (const auto menuButton = dynamic_cast<CustomButton*>(NodeUtils::getNodeByName(mRoot, "menuBtn"))) {
        menuButton->addClickEventListener([](ax::Object*) {
            AXLOG("Back to menu.");
    ...
    if (const auto resetButton = dynamic_cast<CustomButton*>(NodeUtils::getNodeByName(mRoot, "resetBtn"))) {
        resetButton->addClickEventListener([this](ax::Object*) {
            AXLOG("Reset board.");
            if (ax::UserDefault::getInstance()->getBoolForKey("audioEnabled", true)) {
                AudioEngine::play2d("sounds/button.ogg", false);
            }
    ...
    debugUnsolvableBtn->setTitleColor(ax::Color3B::RED);
    ...
    debugUnsolvableBtn->addClickEventListener([&](Object*) {
        resetBoard();
    ...
    debugWinBtn->setTitleColor(ax::Color3B::RED);
    addChild(debugWinBtn);

    debugWinBtn->setPosition({size.width / 2, 50});

    debugWinBtn->addClickEventListener([&](Object*) {
        resetBoard();
...
void GameplayScene::onExit() {
    ...
    if (userWon) {
        AXLOGERROR("User won, generate a new board.");
        // Erase board
        for (int x = 0; x < _gridSizeX; ++x) {
            for (int y = 0; y < _gridSizeY; ++y) {
...
void GameplayScene::generateTile() {
    ...
    if (pos == std::pair<int, int>{-1, -1}) {
        AXLOGERROR("No empty space on the grid.");
        return;
    }
...
void GameplayScene::animateMoveFail(const eDirection dir) const {
    ...
    const ax::Vec2 offset = isHorizontal ? ax::Vec2(8, 0) : ax::Vec2(0, 8);
    constexpr float duration = 0.05f;

    const auto moveR = ax::MoveBy::create(duration, offset);
    const auto moveL = moveR->reverse();
    const auto moveBack = ax::MoveTo::create(duration, bg->getPosition());

    const auto seq = Sequence::create(
        ax::EaseSineInOut::create(moveR),
        ax::EaseSineInOut::create(moveL),
        ax::EaseSineInOut::create(moveR->clone()),
        ax::EaseSineInOut::create(moveL->clone()),
        ax::EaseSineInOut::create(moveBack),
        nullptr
    );
...
void GameplayScene::saveBoard() {
    ...
    ax::UserDefault::getInstance()->setStringForKey("saved_board", output);
    AXLOG("%s", output);
}
...
void GameplayScene::tryLoadBoard() {
    ...
    if (document.Parse(jsonData.data()).HasParseError() || !document.IsArray()) {
        AXLOGERROR("Failed to parse json %d", document.GetParseError());
        resetBoard();
        generateTile();
        generateTile();
        return;
    ...
    for (auto it = array.Begin(); it != array.End(); ++it) {
        if (it->IsObject() && it->HasMember("x") && it->HasMember("y") && it->HasMember("val")) {
            ...
            AXLOGERROR("No value for x,y in array.");
            return;
        }
    }
...
void GameplayScene::updateScore(const int num) {
    // Score update
    mGameScore += num;
    if (const auto gameScore = dynamic_cast<ax::ui::Text*>(NodeUtils::getNodeByName(mRoot, "score"))) {
        gameScore->setString(std::to_string(mGameScore));
    } else {
        AXLOGERROR("No game score node.");
    }

    // Update user score
    if (const auto bestScoreLabel = dynamic_cast<ax::ui::Text*>(NodeUtils::getNodeByName(mRoot, "bestScore"))) {
        int bestScore = ax::UserDefault::getInstance()->getIntegerForKey("best_score", 0);

        // Update best score
        if (mGameScore > bestScore) {
            bestScore = mGameScore;
            ax::UserDefault::getInstance()->setIntegerForKey("best_score", bestScore);
        }
        ...
        if (const auto bestScoreHolder = NodeUtils::getNodeByName(mRoot, "bestScoreHolder")) {
            ...
            bestScoreHolder->setContentSize(ax::Size(totalWidth, bestScoreHolder->getContentSize().height));
        }
    } else {
        AXLOGERROR("No game score.");
    }
}
